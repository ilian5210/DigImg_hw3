...
def sobel_gradients(image):
    ...
    angle = np.arctan2(grad_y, grad_x) * 180 / np.pi  #法向量
    ...
    for i in range(1, magnitude.shape[0] - 1):  #  rows 邊緣不處理
        for j in range(1, magnitude.shape[1] - 1):  
            ...
            if (0 <= angle[i, j] < 22.5) or (157.5 <= angle[i, j] <= 180):  # 0度  水平相鄰
                ...
                q = magnitude[i + 1, j - 1]  #左下
                r = magnitude[i - 1, j + 1]  #右上
            elif 67.5 <= angle[i, j] < 112.5:  # 90度 垂直相鄰 
                q = magnitude[i + 1, j]  #下
                r = magnitude[i - 1, j]  #上
            elif 112.5 <= angle[i, j] < 157.5:  # 135度 斜向相鄰 (左上右下)
                q = magnitude[i - 1, j - 1]  #左上
                r = magnitude[i + 1, j + 1]  #右下
...
#img5
img_handeling(cv2.imread('imgs/img5.jpg', 0), 30, 65, 'img5')